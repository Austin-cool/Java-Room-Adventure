...
public class RoomAdventure{ // Main class containing game logic
    ...
    private static void handleEat(String noun) { // Handles eatable item within grabbables
        String[] edibleItems = {"muchroom", "cake", "apple",}; // Eatable items
        boolean isEdible = false; 
        for (String item : edibleItems){
            if (noun.equals(item)) { // Check if item is edible
                isEdible = true; // set edible to true
                break; // Break out of the loop
            }
        }

        if (isEdible) {
            System.out.println("You eat the " + noun + ". Yum!");
            // Remove the item from the inventory and room after eating
            for (int i = 0; i < inventory.length; i++) { // loop through inventory
                if (inventory[i] != null && inventory[i].equals(noun)) { // Check if item is in inventory
    ...
    private static void setupGame() { //init game world
        ...
        String[] room1Grabbables = {"key", "apple"}; // items in room 1 you can take
